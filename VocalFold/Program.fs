...
[<EntryPoint>]
let main argv =
    // Create a mutex for single-instance detection
    let mutable createdNew = false
    use mutex = new Mutex(true, "Global\\VocalFoldMutex", &createdNew)

    if not createdNew then
        // Another instance is already running
        MessageBox.Show(
            "VocalFold is already running. Check your system tray.",
            "VocalFold",
